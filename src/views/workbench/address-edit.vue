<template>
    <div class="address-modify">
        ...
        <div class="confirm-btn" @click="saveAddress" v-if="type == 'add'">
            保存地址
        </div>
        <div class="confirm-btn" @click="editAddress" v-if="type == 'edit'">
            保存地址
        </div>
        <!-- 地区popup -->
...
import { Switch as vanSwitch, Popup, Area as vanArea, Toast } from "vant";
export default {
    data() {
        return {
            isPop: false,
    ...
    methods: {
        ...
        getLocalhost(address) {
            ...
            this.address = address
                ...
                .join(" ");
            this.form.provinceCode = address[0].code;
            this.form.cityCode = address[1].code;
            this.form.districtCode = address[2] ? address[2].code : "";
            this.isPop = false;
        },
        async saveAddress() {
            //添加
            this.defaultAddress
                ? (this.form.defaultUse = 1)
                : (this.form.defaultUse = 0);
            if (this.valid()) {
                Toast("请输入完整的信息！", 2000);
                return;
            }
    ...
    async created() {
        ...
        if (this.addressId) {
            this.type = "edit";
            let res = await api.getOneAddress(this.addressId);
            let data = res.result;
            this.form = data;
            this.defaultAddress = data.defaultUse == 1 ? true : false;
            this.address = data.addressName;
        }
        this.form.memberId = this.$store.state.user.userId;
    },
    components: {
        vanSwitch,
        Popup,
